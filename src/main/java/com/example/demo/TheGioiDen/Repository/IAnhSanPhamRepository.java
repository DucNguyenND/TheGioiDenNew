...
public interface IAnhSanPhamRepository extends JpaRepository<AnhSanPham, Integer> {
    @Query(value ="select * from public.anh_san_pham where san_pham_id=:id",nativeQuery = true)
    public List<AnhSanPham> getAnhSanPhamByIdSanPham(@Param("id") Integer id);

    @Modifying(clearAutomatically = true)
    @Query(value ="delete from public.anh_san_pham where san_pham_id=:id",nativeQuery = true)
    ...
    @Modifying(clearAutomatically = true)
    @Query(value ="insert into public.anh_san_pham(link_anh,san_pham_id) values(?1,?2)",nativeQuery = true)

...
public interface IPhanLoaiRepository extends JpaRepository<PhanLoaiDto, Integer> {
    ...
    @Query(value ="insert into public.phan_loai(ten_phan_loai,gia_tien,group_code,group_name,id_san_pham) values(?1,?2,?3,?4,?5)",nativeQuery = true)
    ...
    @Query(value ="select * from public.phan_loai where id_san_pham=?1 and group_code is null",nativeQuery = true)

...
public interface ITheGioiDenRepository  extends JpaRepository<SanPham, Integer> {
    @Query(value ="with tmp as(select count(*) as TOTAL_ELEMENT from SAN_PHAM)select tmp.*,sp.* from public.SAN_PHAM sp join tmp on 1=1 order by MUC_DO_UU_TIEN LIMIT :size OFFSET :page",nativeQuery = true)
    public List<SanPham> getAllSanPham(@Param("size") Integer size,@Param("page") Integer page);

    @Query(value ="with tmp as(select count(*) as TOTAL_ELEMENT from SAN_PHAM where danh_muc_san_pham_id=?1)select tmp.*,sp.* from public.SAN_PHAM sp join tmp on 1=1 and sp.danh_muc_san_pham_id=?1 order by MUC_DO_UU_TIEN LIMIT ?2 OFFSET ?3 ",nativeQuery = true)

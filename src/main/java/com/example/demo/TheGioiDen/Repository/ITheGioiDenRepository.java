...
public interface ITheGioiDenRepository  extends JpaRepository<SanPham, Integer> {
    ...
    @Modifying(clearAutomatically = true)
    @Query(value ="update the_gioi_den_95.public.SAN_PHAM set ten_san_pham=?1,thuong_hieu=?2,ma_sp=?3,gia_sp=?4,dien_ap=?5,cong_suat=?6,chi_so_hoan_mau=?7,tuoi_tho=?8,anh_sang=?9,kich_thuoc=?10,mo_ta=?11,link_anh_chinh=?12,danh_muc_san_pham_id=?13 where id=?14",nativeQuery = true)
